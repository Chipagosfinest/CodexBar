...
The cost area remains subordinate and names each currency's covered days and basis. A public API price calculation says “API value estimate · not billed”; provider-reported and mixed values keep their own labels. The image displays at most two currency rows and points to Copy Stats for any remainder, keeping the model composition and footer in frame. The footer counts distinct providers, rather than account rows. The local source and through-date stay on the export. When model coverage is incomplete, the title says “Known model family mix” and the card explains that some model data is unavailable.
